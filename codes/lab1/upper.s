...
# so - original output string for length calc.
...
# If we got here then the iletter is lower case, so convert it.
	addi t2, t2, ('A'-'a')
cont:	 # end if
...
# Setup the parameters to print our hex number and then call Linux to do it.
	sub a0, a1, s0 		# get the len by subing the pointers
	ld s0, 0(sp) 		# pop s0
	ld ra, 8(sp) 		# pop ra
